...
public class MariaDBDatabaseSession extends SQLDatabaseSession<MariaDBDatabaseSession.Args> {

    public static MariaDBDatabaseSession createSession(IDatabaseSession.Args args) {
        if (!(args instanceof Args)) {
            throw new IllegalArgumentException("Must create session with valid args");
        }

        return new MariaDBDatabaseSession((Args) args);
    }

    private MariaDBDatabaseSession(Args args) {
        super(args);
    }
    ...
    public boolean connect() {
        if (this.logging)
            this.args.getLogger().info("Trying to connect to remote database (MariaDB)...");
